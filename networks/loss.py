...
def calc_str_loss(pred, true, mask_2d, same_chain, negative=False, d_clamp=10.0, A=10.0, gamma=0.95, eps=1e-6):
    '''
...
def calc_pseudo_dih(pred, true, mask_crds, eps=1e-6):
    '''
    ...
    mask = torch.logical_and(mask_crds[:,:-3], mask_crds[:,1:-2])
    mask = torch.logical_and(mask, mask_crds[:,2:-1])
    mask = torch.logical_and(mask, mask_crds[:,3:])
    dih_loss = torch.square(pred_dih - true_dih).sum(dim=-1).reshape(I, B, -1) # (I, B, L')
    dih_loss = (mask[None]*dih_loss).sum() / (I*mask.sum() + eps)
