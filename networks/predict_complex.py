...
import util
from data_loader import MSAFeaturize
...
class Predictor():
    def __init__(self, model_dir=None, device="cuda:0", write_pdb_output=False):
        if model_dir == None:
    ...
    def run_prediction(self, msa_orig, ins_orig, t1d, t2d, xyz_t, xyz, state, out_prefix, L1, n_latent=128, use_trf=False, use_fp32=False):
        ...
        with torch.no_grad():
            ...
            #
            N, L = msa.shape
            seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(msa, ins, p_mask=-1.0)
            #
        ...
        prob = np.sum(prob_s[0].permute(1,2,0).detach().cpu().numpy()[:L1,L1:,:20], axis=-1).astype(np.float16)
...
def get_args():
    ...
    parser.add_argument("-msa", required=True, help="Input joint MSA file")
    parser.add_argument("-out", required=True, help="prefix for output file. [out].npz will be generated.")
    parser.add_argument("-L1", required=True, type=int, help="Length of first protein")
