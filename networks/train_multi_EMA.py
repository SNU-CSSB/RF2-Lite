...
from data_loader import get_train_valid_set, loader_complex, loader_pdb, loader_fb, Dataset, DistilledDataset, DistributedWeightedSampler
...
class Trainer():
    ...
    def calc_loss(self, logit_s, label_s,
                  ...
                  pred, true, mask_crds, mask_2d, same_chain,
                  pred_lddt, idx, unclamp=False, negative=False,
                  w_dist=1.0, w_aa=1.0, w_str=1.0, 
                  w_lddt=1.0, w_blen=1.0, w_bang=1.0, eps=1e-6):
        B, L = true.shape[:2]
        ...
        # Structural loss
        if unclamp:
            tot_str, str_loss = calc_str_loss(pred, true, mask_2d, same_chain, negative=negative, d_clamp=None, gamma=0.8, eps=eps)
        else:
            tot_str, str_loss = calc_str_loss(pred, true, mask_2d, same_chain, negative=negative, d_clamp=10.0, gamma=0.8, eps=eps)
        tot_loss += w_str*tot_str
        loss_s.append(str_loss)
        
        lddt_loss, ca_lddt = calc_lddt_loss(pred[:,:,:,1], true[:,:,1], pred_lddt, idx, mask_crds, mask_2d, same_chain, negative=negative, eps=eps)
        tot_loss += w_lddt*lddt_loss
        loss_s.append(lddt_loss.detach()[None])
        loss_s.append(ca_lddt.detach())
        
        # bond geometry
        blen_loss, bang_loss = calc_BB_bond_geom(pred[-1], true, mask_crds, eps=eps)
        if w_blen > 0.0:
        ...
        # pseudo dihedral
        dih_loss = calc_pseudo_dih(pred[:,:,:,1], true[:,:,1], mask_crds, eps=eps)
        tot_loss += dih_loss
    ...
    def train_model(self, rank, world_size):
        ...
        compl_IDs, compl_weights, compl_dict, neg_IDs, neg_weights, neg_dict, pdb_IDs, pdb_weights, pdb_dict, fb_IDs, fb_weights, fb_dict, valid_dict = get_train_valid_set(self.loader_param)
        self.n_train = 4800*8
        valid_ids = list(valid_dict.keys())
        random.shuffle(valid_ids)
        self.n_valid = len(valid_ids)
        self.n_valid = (self.n_valid // world_size)*world_size
        
        train_set = DistilledDataset(compl_IDs, loader_complex, compl_dict, \
                                     neg_IDs, loader_complex, neg_dict, \
                                     pdb_IDs, loader_pdb, pdb_dict, \
                                     fb_IDs, loader_fb, fb_dict, \
                                     self.loader_param)
        valid_set = Dataset(valid_ids[:self.n_valid], loader_complex, valid_dict, self.loader_param)
        #
        train_sampler = DistributedWeightedSampler(train_set, compl_weights, neg_weights, pdb_weights, fb_weights, 
                                                   epoch_unit=4800,
        ...
        #    if "iter_block_2.3.msa2pair.proj_updt" in n:
        #        print (n, p)
        ...
        valid_tot, valid_loss, valid_acc = self.valid_cycle(ddp_model, valid_loader, rank, gpu, world_size, loaded_epoch)
    ...
    def train_cycle(self, ddp_model, train_loader, optimizer, scheduler, scaler, rank, gpu, world_size, epoch):
        ...
        for seq, msa_orig, msa_seed, msa_extra, mask_msa, true_crds, mask_crds, res_idx, chain_idx, unclamp, negative in train_loader:
            # transfer inputs to device
            B, _, N, L = msa_orig.shape
            
            res_idx = res_idx.to(gpu, non_blocking=True) # (B, L)
            chain_idx = chain_idx.to(gpu, non_blocking=True) # (B, L, L) -- if two residues are in same chain = 1, otherwise 0
            true_crds = true_crds.to(gpu, non_blocking=True) # (B, L, 3, 3)
            mask_crds = mask_crds.to(gpu, non_blocking=True) # (B, L)

            seq = seq.to(gpu, non_blocking=True) # (B, MAX_CYCLE, L)
            msa_orig = msa_orig.to(gpu, non_blocking=True) # (B, MAX_CYCLE, N, L)
            msa_seed = msa_seed.to(gpu, non_blocking=True) # (B, MAX_CYCLE, N_SEED, L, C1)
            msa_extra = msa_extra.to(gpu, non_blocking=True) # (B, MAX_CYCLE, N_EXTRA, L, C2)
            mask_msa = mask_msa.to(gpu, non_blocking=True) # (B, MAX_CYCLE, N_SEED, L)
            
            # processing labels 
            mask_2d = mask_crds[:,None,:] * mask_crds[:,:,None] # ignore pairs having missing residues
            c6d, _ = xyz_to_c6d(true_crds)
            c6d = c6d_to_bins2(c6d, chain_idx, negative=negative)

            # make fake template
            xyz_t = torch.full((B, 1, L, 3, 3), np.nan, device=seq.device).float()
            t1d = torch.nn.functional.one_hot(torch.full((B,1,L),20).long(), num_classes=21).float()
            t1d = torch.cat((t1d, torch.zeros((B, 1, L, 1)).float()), -1)
            t1d = t1d.to(gpu)
                ...
                for i_cycle in range(N_cycle-1):
                    with ddp_model.no_sync():
                        with torch.cuda.amp.autocast(enabled=USE_AMP):
                            msa_prev, pair_prev, xyz_prev, logit_aa_s, lddt_prev = ddp_model(msa_seed[:,i_cycle],
                                                                      msa_extra[:,i_cycle],
                                                                      seq[:,i_cycle], xyz_prev, state,
                                                                      res_idx,
            ...
            if counter%self.ACCUM_STEP != 0:
                with ddp_model.no_sync():
                    with torch.cuda.amp.autocast(enabled=USE_AMP):
                        logit_s, logit_aa_s, pred_crds, pred_lddts = ddp_model(msa_seed[:,i_cycle],
                                                                   msa_extra[:,i_cycle],
                                                                   seq[:,i_cycle], xyz_prev, state, 
                                                                   res_idx,
                                                                   t1d=t1d, t2d=t2d, xyz_t=xyz_t,
                        ...
                        acc_s, _, _ = self.calc_acc(prob, c6d[...,0], res_idx, mask_2d)

                        loss, loss_s = self.calc_loss(logit_s, c6d,
                                logit_aa_s, msa_orig[:, i_cycle], mask_msa[:,i_cycle],
                                pred_crds, true_crds, mask_crds, mask_2d, chain_idx,
                                pred_lddts, res_idx, unclamp=unclamp, negative=negative, **self.loss_param)
    ...
    def valid_cycle(self, ddp_model, valid_loader, rank, gpu, world_size, epoch):
        ...
        with torch.no_grad(): # no need to calculate gradient
            ...
            for seq, msa_orig, msa_seed, msa_extra, mask_msa, true_crds, mask_crds, res_idx, chain_idx, unclamp, negative in valid_loader:
                # transfer inputs to device
                B, _, N, L = msa_orig.shape
                ...
                logit_s, logit_aa_s, pred_crds, pred_lddts = ddp_model(msa_seed[:,i_cycle],
                                                           ...
                                                           use_checkpoint=False)
                prob = self.active_fn(logit_s[0]) # distogram
                acc_s, cnt_pred, cnt_ref = self.calc_acc(prob, c6d[...,0], res_idx, mask_2d)

                # inter-chain contact prob
                cnt_pred = cnt_pred * (1-chain_idx).float()
                cnt_ref = cnt_ref * (1-chain_idx).float()
                ...
                if max_prob > 0.9:
                    ...
                    if (cnt_ref > 0).any():
                        ...
                        TN += 1.0
                inter_s = torch.tensor([TP/(TP+FP+1e-4), TP/(TP+FN+1e-4), 2*TP/(2*TP+FP+FN+1e-4)], device=prob.device).float()
        ...
        valid_inter /= float(counter*world_size)
        ...
        if rank == 0:
            ...
            train_time = time.time() - start_time
            sys.stdout.write("Valid: [%04d/%04d] Batch: [%05d/%05d] Time: %16.1f | total_loss: %8.4f | %s | %.4f %.4f %.4f | %.4f %.4f %.4f\n"%(\
                    ...
                    valid_inter[0], valid_inter[1], valid_inter[2])) 
            sys.stdout.flush()
