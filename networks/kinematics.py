...
def get_init_xyz(xyz_t):
    # input: xyz_t (B, T, L, 3, 3)
    # ouput: xyz (B, T, L, 3, 3)
    B, T, L = xyz_t.shape[:3]
    init = INIT_CRDS.to(xyz_t.device).reshape(1,1,1,3,3).repeat(B,T,L,1,1)
    if torch.isnan(xyz_t).all():
        return init.contiguous()

    mask = torch.isnan(xyz_t).any(dim=-1).any(dim=-1) # (B, T, L)
    #
    xyz_t = torch.nan_to_num(xyz_t)
    center_CA = ((~mask[:,:,:,None]) * xyz_t[:,:,:,1,:]).sum(dim=2) / ((~mask[:,:,:,None]).sum(dim=2)+1e-4) # (B, T, 3)
