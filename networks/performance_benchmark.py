...
class Predictor():
    ...
    def predict(self, a3m_fn, out_prefix, L1, n_latent=128, use_trf=False, use_fp32=False):
        ...
        for i_trial in range(1):
            ...
            print ("Memory used:", max_mem, "/ Time: %.2f sec"%(time.time()-start_time), "/ for (%d,%d) input"%(N, L))
    ...
    def run_prediction(self, msa_orig, ins_orig, t1d, t2d, xyz_t, xyz, state, out_prefix, L1, n_latent=128, use_trf=False, use_fp32=False):
        ...
        with torch.no_grad():
            ...
            seq, msa_seed_orig, msa_seed, msa_extra, mask_msa = MSAFeaturize(msa, ins,\
                        params={'MAXLAT':128, 'MAXSEQ': 1024, "MAXCYCLE": 3}, p_mask=-1.0)
            #
            ...
            for i_cycle in range(MAX_CYCLE):
                ...
                best_logit = logit_s[0]
                best_lddt = pred_lddt.clone()
                torch.cuda.empty_cache()
       
        prob = self.active_fn(best_logit.float())
        ...
        prob = np.sum(prob.permute(1,2,0).detach().cpu().numpy()[:L1,L1:,:20], axis=-1).astype(np.float16)
