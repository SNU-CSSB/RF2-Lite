...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, x, idx):
        ...
        x += emb # add relative positional encoding
        return self.drop(x)
...
class MSA_emb(nn.Module):
    ...
    def forward(self, msa, seq, idx):
        ...
        msa += tmp.expand(-1, N, -1, -1) # adding query embedding to MSA
        msa = self.drop(msa)
...
class Extra_emb(nn.Module):
    ...
    def forward(self, msa, seq, idx):
        ...
        msa += seq.expand(-1, N, -1, -1) # adding query embedding to MSA
...
class Templ_emb(nn.Module):
    ...
    def forward(self, t1d, t2d, xyz_t, pair, use_checkpoint=False):
        # Input
        ...
        templ = self.templ_stack(templ, xyz_t, use_checkpoint=use_checkpoint) # template templure
...
class Recycling(nn.Module):
    def __init__(self, d_msa=256, d_pair=128):
        ...
        self.proj_dist = nn.Linear(36+44, d_pair)
